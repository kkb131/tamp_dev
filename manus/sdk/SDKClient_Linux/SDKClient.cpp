...
ClientReturnCode SDKClient::Run()
{
	...
	while (!m_RequestedExit)
	{
		...
		case ClientState::ClientState_DisplayingData:
		{
			...
			else if (m_CurrentInteraction == nullptr)
			{
...
ClientReturnCode SDKClient::InitializeSDK()
{
	if (m_IsHeadless)
	{
		// Headless mode: auto-select Integrated
...
ClientReturnCode SDKClient::DisplayingData()
{
	...
	GO_TO_DISPLAY('L', DisplayingDongleInfo);
	GO_TO_DISPLAY('F', DisplayingFirmwareInfo);
...
ClientReturnCode SDKClient::DisplayingDataTracker()
{
	...
	ClientLog::print("[O] Toggle Test Tracker [G] Toggle per user tracker display [T] Set Tracker Offset to Left hand MANUS Universal mount");
...
ClientReturnCode SDKClient::DisplayingDongleInfo()
{
	ClientLog::print("[Q] Back  <<Dongle Info>> [ESC] quit");
	ClientLog::print("<Dongle Info>[C] Clear License [S] Set license");

	AdvanceConsolePosition(3);

	GO_TO_MENU_IF_REQUESTED();

	HandleDongleCommands();
...
ClientReturnCode SDKClient::DisplayingFirmwareInfo()
{
	ClientLog::print("[Q] Back  <<Firmware Info>> [ESC] quit");
	ClientLog::print("<Firmware Info>[D] Update dongle firmware [G] Update glove firmware");

	AdvanceConsolePosition(2);

	GO_TO_MENU_IF_REQUESTED();

	HandleFirmwareCommands();
...
/// @brief Stream ergonomics data as newline-delimited JSON to stdout.
/// Called in headless (--stream-json) mode instead of interactive display.
...
void SDKClient::StreamErgonomicsAsJSON()
{
	...
	auto t_OutputHand = [&](const char* p_Hand, ErgonomicsData& p_Ergo, uint32_t p_GloveID, int p_Offset)
	{
		...
		{
			if (j > 0) std::cout << ",";
			std::cout << std::fixed << std::setprecision(2) << p_Ergo.data[p_Offset + j];
		}
		std::cout << "],\"finger_spread\":[";
		for (int f = 0; f < 5; f++)
		{
			if (f > 0) std::cout << ",";
	...
	// Left hand: data[0:20]
	t_OutputHand("left", m_LeftGloveErgoData, m_FirstLeftGloveID, 0);
	// Right hand: data[20:40]
	t_OutputHand("right", m_RightGloveErgoData, m_FirstRightGloveID, 20);
}
...
void SDKClient::PrintLicenseInfo()
{
	LicenseInfo t_LicenseInfo = m_Landscape->settings.license;
	ClientLog::printWithPadding("Max Glove Pairs:   {}", 5, static_cast<uint16_t>(t_LicenseInfo.maxGlovePairs));
	ClientLog::printWithPadding("Recording:         {}", 5, t_LicenseInfo.recording);
	ClientLog::printWithPadding("Exporting:         {}", 5, t_LicenseInfo.exporting);
	ClientLog::printWithPadding("Advance Exporting: {}", 5, t_LicenseInfo.advancedExporting);
	ClientLog::printWithPadding("Unity              {}", 5, t_LicenseInfo.unitySession);
	ClientLog::printWithPadding("Unreal:            {}", 5, t_LicenseInfo.unrealSession);
	ClientLog::printWithPadding("OpenXR:            {}", 5, t_LicenseInfo.openXRSession);
	ClientLog::printWithPadding("SDK:               {}", 5, t_LicenseInfo.sdk);
	ClientLog::printWithPadding("Raw:               {}", 5, t_LicenseInfo.raw);
	ClientLog::printWithPadding("Mobu:              {}", 5, t_LicenseInfo.mobuSession);
	ClientLog::printWithPadding("Xsens:             {}", 5, t_LicenseInfo.xsensSession);
	ClientLog::printWithPadding("Optitrack:         {}", 5, t_LicenseInfo.optitrackSession);
	ClientLog::printWithPadding("Qualisys:          {}", 5, t_LicenseInfo.qualisysSession);
	ClientLog::printWithPadding("Vicon:             {}", 5, t_LicenseInfo.viconSession);
	ClientLog::printWithPadding("Nokov:             {}", 5, t_LicenseInfo.nokovSession);
	ClientLog::printWithPadding("Icido:             {}", 5, t_LicenseInfo.icidoSession);
	ClientLog::printWithPadding("Siemens:           {}", 5, t_LicenseInfo.siemensSession);
	ClientLog::printWithPadding("Vred:              {}", 5, t_LicenseInfo.vredSession);
	ClientLog::printWithPadding("Integrated:        {}", 5, t_LicenseInfo.integrated);
	ClientLog::printWithPadding("Cortex:            {}", 5, t_LicenseInfo.cortexSession);
	ClientLog::printWithPadding("FZ Motion:         {}", 5, t_LicenseInfo.fzMotionSession);
	ClientLog::printWithPadding("WorldViz:          {}", 5, t_LicenseInfo.worldVizSession);
	ClientLog::printWithPadding("Noraxon:           {}", 5, t_LicenseInfo.noraxonSession);
	ClientLog::printWithPadding("Noitom:            {}", 5, t_LicenseInfo.noitomSession);
	ClientLog::printWithPadding("Ableton:           {}", 5, t_LicenseInfo.abletonSession);

	AdvanceConsolePosition(25);
}
...
/// @brief This support function is used to set the tracker offset. It sets the tracker offset for the left hand tracker to MANUS Universal mount center point.
void SDKClient::SetTrackerOffset() {

	TrackerOffset t_TrackerOffset;

	//Positions the tracker 3mm right, 58mm up and 43mm forward relative to the MANUS Universal mount center point. This value is illustrative and should be adjusted to the actual measurement.
	t_TrackerOffset.translation = { 0.003f, -0.058f, -0.043f };
	t_TrackerOffset.rotation = { 1.0f, 0.0f, 0.0f, 0.0f };
	t_TrackerOffset.entryType = TrackerOffsetType::TrackerOffsetType_LeftHandTrackerToCasing;
...
void SDKClient::HandleGloveCalibrationCommands()
{
	...
	{
		uint32_t t_Size = 0;
		if (CoreSdk_GetGloveCalibrationSize(m_CalibrationGloveId, &t_Size) != SDKReturnCode_Success) return;
		unsigned char* t_CalibrationBytes = new unsigned char[t_Size];
		if (CoreSdk_GetGloveCalibration(t_CalibrationBytes, t_Size) != SDKReturnCode_Success) return;
...
void SDKClient::HandleDongleCommands()
{
	// Get dongle id
	if (m_Landscape == nullptr || m_Landscape->gloveDevices.dongleCount == 0)
		return;

	auto t_DongleData = m_Landscape->gloveDevices.dongles[0];

	if (GetKeyDown('C'))
	{
		bool t_Succes;
		char t_Response[MAX_NUM_CHARS_IN_RESPONSE];
		if (CoreSdk_ClearLicense(t_DongleData.id, &t_Succes, t_Response) == SDKReturnCode_Success)
		{
			if (t_Succes)
				ClientLog::print("Cleared license");
			else
				ClientLog::print(t_Response);
		}
		else
		{
			ClientLog::warn("Something went wrong clearing the license");
		}
		AdvanceConsolePosition(2);
	}
	...
	if (GetKeyDown('S'))
	{
		...
		// get temporary skeleton data from file
		char* t_LicenseData = new char[t_FileLength];
		t_File.read((char*)t_LicenseData, t_FileLength);
		t_File.close();

		bool t_Succes;
		char t_Response[MAX_NUM_CHARS_IN_RESPONSE];
		if (CoreSdk_SetLicense(t_DongleData.id, t_LicenseData, t_FileLength, &t_Succes, t_Response) == SDKReturnCode_Success)
		{
			if (t_Succes)
				ClientLog::print("License set successfully");
			else
				ClientLog::print(t_Response);
		}
		else
		{
			ClientLog::warn("Something went wrong setting the license");
		}
...
void SDKClient::HandleFirmwareCommands()
{
	// Get landscape
	if (m_Landscape == nullptr)
		return;
	...
	if (GetKeyDown('D'))
	{
		...
		uint32_t t_DongleID = m_Landscape->gloveDevices.dongles[0].id;

		bool t_Result;
		char t_Response[MAX_NUM_CHARS_IN_RESPONSE];
		if (CoreSdk_UpdateFirmware(t_DongleID, &t_Result, t_Response) == SDKReturnCode_Success)
		{
			ClientLog::print(t_Response);
		}
		else
		{
			ClientLog::warn("Something went wrong updating the firmware");
		}
		AdvanceConsolePosition(2);
	}
	...
	if (GetKeyDown('G'))
	{
		if (m_Landscape->gloveDevices.gloveCount == 0)
			return;

		uint32_t t_GloveID = m_Landscape->gloveDevices.gloves[0].id;

		bool t_Result;
		char t_Response[MAX_NUM_CHARS_IN_RESPONSE];
		if (CoreSdk_UpdateFirmware(t_GloveID, &t_Result, t_Response) == SDKReturnCode_Success)
		{
			ClientLog::print(t_Response);
		}
		else
